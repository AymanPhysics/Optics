...
Public Class RPT2
    ...
    Public Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles Button2.Click, btnPDF.Click
        ...
        If Not (Flag = 1 AndAlso Md.MyProjectType = ProjectType.X25) Then
        ...
        If Val(MainAccNo.Text) = Val(bm.ExecuteScalar("select YearProfitAccNo from Statics")) Then
        ...
        rpt.paraname = New String() {"@MainAccNo", "MainAccName", "@SubAccNo", "SubAccName", "@FromDate", "@ToDate", "Header", "@Detailed", "@DetailedInvoice", "@LinkFile", "@ToId", "@RPTFlag1", "@RPTFlag2", "@ActiveOnly", "@HasBalOnly", "@WindowId", "@CostCenterId", "@CostCenterSubId", "@MainAccNo", "@FromMainAccNo", "@ToMainAccNo"}
        rpt.paravalue = New String() {MainAccNo.Text, MainAccName.Text, Val(SubAccNo.Text), SubAccName.Text, FromDate.SelectedDate, ToDate.SelectedDate, CType(Parent, Page).Title.Trim & " " & IIf(MainLinkFile.SelectedIndex > 0, MainLinkFile.Text, ""), IIf(Detailed.IsChecked, 1, 0), IIf(DetailedInvoice.IsChecked, 1, 0), MyLinkFile, Val(SubAccNo.Text), RPTFlag1, 0, IIf(Detailed.IsChecked, 1, 0), IIf(DetailedInvoice.IsChecked, 1, 0), Val(WindowId.SelectedValue), Val(CostCenterId.SelectedValue), Val(CostCenterSubId.SelectedValue), MainAccNo.Text, MainAccNo.Text, ToMainAccNo.Text}
        Select Case Flag
            Case 1
                ...
                If Detailed.IsChecked AndAlso Md.MyProjectType = ProjectType.X13 Then rpt.Rpt = "AccountMotion4.rpt"
                If Md.MyProjectType = ProjectType.Optics Then rpt.Rpt = "AccountMotionCurrency.rpt"
            ...
            Case 4
                rpt.Rpt = "Sales2_O2.rpt"
        End Select
        If sender Is btnPDF Then
            rpt.ReportViewer_Load(Nothing, Nothing)
            Dim fileName As String = bm.GetNewTempName(SubAccName.Text & ".pdf")
            rpt.ReportDoc.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, fileName)
            Process.Start("whatsapp://send?phone=2" & bm.ExecuteScalar("select Mobile from customers where Id=" & SubAccNo.Text))
            Process.Start("explorer.exe", "/select," & fileName)
            'bm.SendEMail(CompanyName, "<H1>" & CompanyName & "</H1> <br/> <H3>Your ID is  <br/> " & SubAccNo.Text & "</H3>", fileName, {bm.ExecuteScalar("select Address from customers where Id=" & SubAccNo.Text)})
        Else
            rpt.Show()
        End If
    End Sub
    ...
    Private Sub UserControl_Loaded(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles MyBase.Loaded
        ...
        ElseIf Flag = 3 Then
            'Detailed.Visibility = Windows.Visibility.Hidden
            Detailed.Content = "له حركات"
            DetailedInvoice.Content = "له رصيد"

            Detailed.IsChecked = True
            DetailedInvoice.IsChecked = True
        ...
        If MyLinkFile > 0 And Flag <> 3 And Flag <> 4 Then
        ...
        If Flag = 1 AndAlso Md.MyProjectType = ProjectType.X25 Then
        ...
        Else
            lblToMainAcc.Visibility = Windows.Visibility.Hidden
            ToMainAccNo.Visibility = Windows.Visibility.Hidden
            ToMainAccName.Visibility = Windows.Visibility.Hidden
    ...
    Private Sub MainAccNo_LostFocus(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles MainAccNo.LostFocus
        ...
        SubAccNo.IsEnabled = MainAccNo.Visibility <> Windows.Visibility.Visible OrElse MyLinkFile > 0 OrElse bm.ExecuteAdapter("select * from LinkFile where Id=(select C.LinkFile from Chart C where C.Id='" & MainAccNo.Text & "')").Rows.Count > 0
        SubAccNo_LostFocus(Nothing, Nothing)
    End Sub
    ...
    Private Sub ToMainAccNo_LostFocus(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles ToMainAccNo.LostFocus
        bm.AccNoLostFocus(ToMainAccNo, ToMainAccName, , MyLinkFile, )
    End Sub
    ...
    Private Sub ToMainAccNo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles ToMainAccNo.KeyUp
        bm.AccNoShowHelp(ToMainAccNo, ToMainAccName, e, , MyLinkFile, )
    End Sub
