...
Public Class SalesRed
    ...
    Private Sub Sales_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        bm.TestSecurity(Me, {btnSave}, {btnDelete}, {btnFirst, btnNext, btnPrevios, btnLast}, {btnPrint, btnPrint2, btnPrint3, btnPrint4, btnPrint5})
        LoadResource()
        ...
        If Not Flag = FlagState.مشتريات OrElse Not Md.MyProjectType = ProjectType.ClinicPublicElSadaka Then
            btnPrint4.Visibility = Windows.Visibility.Hidden
        End If
        'CashierId.IsEnabled = Md.Manager = 1
        If Not (Md.MyProjectType = ProjectType.Clothes) Then
        ...
        If Md.MyProjectType = ProjectType.Clothes Then
            GroupBoxPayment.Visibility = Windows.Visibility.Hidden
    ...
    Private Sub LoadWFH()
        ...
        If Not ((Md.MyProjectType = ProjectType.Clothes) AndAlso TestPurchaseOnly()) Then
    ...
    Function ReadOnlyState() As Boolean
        If Md.Manager Then
            Return False
        ElseIf Md.MyProjectType = ProjectType.ContractRaggal OrElse Md.MyProjectType = ProjectType.Perfume OrElse Md.MyProjectType = ProjectType.Market OrElse Md.MyProjectType = ProjectType.MarketWithoutShifts Then
            Return False
        Else
            Return False
        End If
    ...
    Sub LoadAllItems()
        Try
            ...
            If Md.Receptionist AndAlso Md.MyProjectType = ProjectType.Clothes Then
...
Br:
    ...
    Private Sub GridCalcRow(ByVal sender As Object, ByVal e As Forms.DataGridViewCellEventArgs)
        Try
            If G.Columns(e.ColumnIndex).Name = GC.Barcode AndAlso Not G.Rows(e.RowIndex).Cells(GC.Barcode).Value Is Nothing Then
                ...
                If (Md.MyProjectType = ProjectType.Clothes) AndAlso Not G.Rows(e.RowIndex).Cells(GC.Barcode).Value = Nothing Then
                    BC = BC.Substring(1)
    ...
    Sub ClearControls()
        Try
            ...
            If Md.MyProjectType <> ProjectType.ClinicPublicElSadaka AndAlso Md.MyProjectType <> ProjectType.APSD AndAlso Md.MyProjectType <> ProjectType.APS Then CashierId.Text = Md.UserName
            ...
            If (Md.MyProjectType = ProjectType.Clothes) AndAlso TestSalesOnly() Then
                Temp.Content = "فاتورة حجز"
                Temp.Visibility = Windows.Visibility.Hidden
            ElseIf (Md.MyProjectType = ProjectType.Clothes) AndAlso TestPurchaseOnly() Then
    ...
    Private Sub PrintPone(ByVal sender As System.Object, ByVal NewItemsOnly As Integer)
        ...
        If NewItemsOnly = 0 Then
            ...
            rpt.Rpt = IIf(Md.MyProjectType = ProjectType.NawarGroup, "SalesPone_N.rpt", "SalesPone.rpt")
            'If sender Is btnPrint2 Then rpt.Rpt = "SalesPone2.rpt"
            If sender Is btnPrint3 Then rpt.Rpt = IIf(Md.MyProjectType = ProjectType.NawarGroup, "SalesPone3_N.rpt", "SalesPone3.rpt")
            If sender Is btnPrint4 Then rpt.Rpt = IIf(Md.MyProjectType = ProjectType.NawarGroup, "SalesPone4_N.rpt", "SalesPone4.rpt")

            If sender Is btnPrint2 Then
                Dim i As Integer = 1
                rpt.Rpt = IIf(Md.MyProjectType = ProjectType.NawarGroup, "SalesPone2_N.rpt", "SalesPone2.rpt")
                If Md.MyProjectType = ProjectType.Clothes Then
    ...
    Private Sub Temp_Checked(sender As Object, e As RoutedEventArgs) Handles Temp.Checked, Temp.Unchecked
        If Temp.IsChecked Then
            btnResSearch.Visibility = Windows.Visibility.Visible
            btnResEnd.Visibility = Windows.Visibility.Visible
            btnDelete.Visibility = Windows.Visibility.Visible
        Else
            btnResSearch.Visibility = Windows.Visibility.Hidden
            btnResEnd.Visibility = Windows.Visibility.Hidden
            If Not Md.Manager Then
                btnDelete.Visibility = Windows.Visibility.Hidden
            End If
        End If
