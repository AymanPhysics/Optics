...
Public Class BasicMethods
    ...
    Public Function CostCenterIdShowHelp(ByVal txtId As TextBox, ByVal txtName As TextBox, ByVal e As System.Windows.Input.KeyEventArgs, Optional ByVal SubType As Integer = 1, Optional MyTableName As String = "CostCenters") As Boolean
    ...
    Public Function ShowHelpGridItemBal(ByVal Column1 As Forms.DataGridViewCell, ByVal Column2 As Forms.DataGridViewCell, ByVal e As Forms.KeyEventArgs, ByVal Statement As String, Optional ByVal TableName As String = "") As Boolean
        If Md.MyProjectType = ProjectType.X14 AndAlso e.KeyCode = Forms.Keys.F12 Then
            Dim hh As New Help
            hh.Header = "الأرصدة"
            hh.Statement = Statement
            hh.TableName = TableName
    ...
    Public Function ShowHelpCases(ByVal txtId As TextBox, ByVal txtName As TextBox, ByVal e As System.Windows.Input.KeyEventArgs, Optional ExistsOnly As Boolean = False, Optional AllowAdd As Boolean = True, Optional ForceAllowAdd As Boolean = False) As Boolean
        If Md.MyProjectType = ProjectType.X2 Then AllowAdd = False
        If e.Key = System.Windows.Input.Key.F1 Then
            If Md.MyProjectType = ProjectType.X1 Then
    ...
    Public Function ShowHelpCases2(ByVal txtId As TextBox, ByVal txtName As TextBox, ByVal e As System.Windows.Input.KeyEventArgs) As Boolean
        If e.Key = System.Windows.Input.Key.F1 Then
            Dim hh As New HelpCases
...
A:
    ...
    Dim MyReceivers() As String
    Public Sub SendEMail(Subject As String, Body As String, ByVal Bath As String, Receivers() As String)
        Dim demoThread As New System.ComponentModel.BackgroundWorker
        AddHandler demoThread.DoWork, AddressOf MyDoWrok0
    ...
    End Sub
    Sub MyDoWrok0(ByVal sender As Object, ByVal ev As EventArgs)
        SendEMailToReceivers(MySubject, MyBody, MyBath, MyReceivers)
    End Sub

    Public Sub SendEMailToReceivers(Subject As String, Body As String, ByVal Bath As String, Receivers() As String)
        'https://myaccount.google.com/lesssecureapps

        Dim mailMsg As New MailMessage()
        mailMsg.From = New MailAddress(Md.gmail, CompanyName)
        ...
        client.Credentials = New NetworkCredential(Md.gmail, Md.gmailPassword)
    ...
    Sub BackupToBath()
        Try
            ...
            If Result = Forms.DialogResult.OK Then
                Dim MyPath As String = ExecuteScalar("bk", New String() {"Bath"}, New String() {dialog.SelectedPath})
    ...
    Sub RestoreFromBath()
        ...
        OFD.Filter = "Bacukps (*.bak)|*.bak|All files (*.*)|*.*"
        If OFD.ShowDialog = Windows.Forms.DialogResult.OK Then
            Dim db_name As String = ExecuteScalar("select db_name()")

            Dim name1 = ExecuteScalar("Select name from sys.database_files where type=0")
            Dim physical_name1 = ExecuteScalar("Select physical_name from sys.database_files where type=0")
            Dim name2 = ExecuteScalar("Select name from sys.database_files where type=1")
            Dim physical_name2 = ExecuteScalar("Select physical_name from sys.database_files where type=1")

            ExecuteNonQuery("use master" & vbCrLf & "ALTER DATABASE [" & db_name & "] SET  SINGLE_USER WITH ROLLBACK IMMEDIATE" & vbCrLf & "RESTORE DATABASE [" & db_name & "] FROM  DISK = N'" & OFD.FileName & "' WITH  FILE = 1,MOVE N'" & name1 & "' TO N'" & physical_name1 & "',MOVE N'" & name2 & "' TO N'" & physical_name2 & "',NOUNLOAD,  REPLACE,  STATS = 10", False)
            ShowMSG("تم استرجاع البيانات")
        End If
    End Sub
    ...
    Public Function ExecuteAdapter(ByVal StoredName As String, ByVal ParaName() As String, ByVal ParaValue() As String, ByVal Type() As SqlDbType) As DataTable
    ...
    Public Function ExecuteNonQuery(ByVal StoredName As String, ByVal ParaName() As String, ByVal ParaValue() As Object, ByVal Type() As SqlDbType) As Boolean
    ...
    Function GetNewTempName(ByVal FileName As String) As String
        ...
        While True
            i += 1
            s = Path & FileName.Split(".").First & IIf(i = 1, "", " (" & i & ")") & "." & FileName.Split(".").Last
...
A:
    ...
    Sub TestSecurity(frm As Object, EditControls() As Control, DeleteControls() As Control, NavigateControls() As Control, PrintControls() As Control)
