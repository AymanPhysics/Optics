...
Public Class Sales
    ...
    Public MyStoreId As Integer
    Public MyInvoiceNo As Integer
    ...
    Public Sub Sales_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        If Md.MyProjectType = ProjectType.X4 OrElse Md.MyProjectType = ProjectType.X5 Then
            btnNew.Content &= " (F8)"
        ...
        If Not Md.MyProjectType = ProjectType.X24 Then
        ...
        If Md.MyProjectType = ProjectType.X17 AndAlso Flag = FlagState.المبيعات Then
    ...
    Structure GC
        ...
        Shared CustVal As String = "CustVal "
        Shared CompVal As String = "CompVal"
    End Structure
    ...
    Private Sub LoadWFH()
        ...
        G.Columns.Add(GC.CustVal, "تحمل العميل")
        G.Columns.Add(GC.CompVal, "تحمل الشركة")
        G.Columns(GC.CompVal).ReadOnly = True
        ...
        'If Not Md.Manager AndAlso Not Md.EditPrices AndAlso (Md.MyProjectType = ProjectType.ClinicPublicElSadaka OrElse Md.MyProjectType = ProjectType.APSD OrElse Md.MyProjectType = ProjectType.APS) AndAlso TestConsumablesAndReturn() Then
        ...
        Else
            G.Columns(GC.CustVal).Visible = False
            G.Columns(GC.CompVal).Visible = False
        End If
        ...
        If Md.MyProjectType = ProjectType.X17 AndAlso Flag = FlagState.تحويل_إلى_مخزن Then
        ...
        If Not (Md.MyProjectType = ProjectType.X24 OrElse Md.MyProjectType = ProjectType.X25) Then
        ...
        If Not (Md.MyProjectType = ProjectType.Optics AndAlso TestImportOnly()) Then
    ...
    Function ReadOnlyState() As Boolean
        If Md.MyProjectType = ProjectType.X17 AndAlso (Flag = FlagState.تسوية_صرف OrElse Flag = FlagState.صرف) Then
            Return True
        ElseIf Md.Manager OrElse Md.EditPrices Then
            Return False
        ElseIf Md.MyProjectType = ProjectType.X13 OrElse Md.MyProjectType = ProjectType.X16 Then
            Return False
    ...
    Function ItemWhere() As String
        ...
        If Md.MyProjectType = ProjectType.X17 AndAlso TestSalesAndReturn() Then
    ...
    Private Sub LoadItems(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Try
            ...
            Dim dt As DataTable = bm.ExecuteAdapter("Select * From Items_View  where IsStopped=0 " & ItemWhere() & " and GroupId=" & WTypes.Tag.ToString & " and TypeId=" & xx.Tag.ToString & " order by " & IIf(Md.MyProjectType = ProjectType.X24, "Id", "Name"))
    ...
    Sub Load_dv_items()
        WItems.Children.Clear()
        dv_items.RowFilter = "Name like '%" & txtName2.Text.Trim & "%'"
        For Each rowView As DataRowView In dv_items
            Dim row As DataRow = rowView.Row
            Dim x As New Button
            bm.SetStyle(x, 370)
            'bm.SetImage(x, CType(dt.Rows(i)("Image"), Byte()))
            x.Tag = row("Id").ToString
            x.Content = row("Name").ToString
            x.ToolTip = row("Name").ToString
    ...
    Function AddItem(ByVal Id As String, Optional ByVal i As Integer = -1, Optional ByVal Add As Decimal = 1) As Integer
        Try
            ...
            If i = -1 Then
                ...
                '        If Md.MyProjectType = ProjectType.RivieraCenter Then
                '            i = x
                '            Exists = True
                '            GoTo Br
                '        Else
...
Br:
            ...
            If (Md.MyProjectType = ProjectType.X17 OrElse Md.MyProjectType = ProjectType.X15) AndAlso (Flag = FlagState.إضافة OrElse Flag = FlagState.مشتريات OrElse Flag = FlagState.الاستيراد OrElse Flag = FlagState.تحويل_إلى_مخزن OrElse Flag = FlagState.المبيعات) Then
                If i = 0 Then
                    Dim x As Integer = 1
                    Select Case Flag
                        Case FlagState.تحويل_إلى_مخزن
        ...
        Try
            ...
            If Not lop AndAlso (Flag = FlagState.المبيعات OrElse Flag = FlagState.مردودات_مشتريات OrElse Flag = FlagState.مردودات_مشتريات_خارجية OrElse Flag = FlagState.تحويل_إلى_مخزن OrElse Flag = FlagState.عرض_أسعار OrElse Flag = FlagState.أمر_توريد OrElse Flag = FlagState.صرف OrElse Flag = FlagState.تسوية_صرف) AndAlso Val(G.Rows(i).Cells(GC.Qty).Value) > Val(G.Rows(i).Cells(GC.CurrentBal).Value) Then
                If G.CurrentCell.ColumnIndex = G.Columns(GC.Qty).Index Then
                    If bm.ExecuteScalar("select IsService from Items where Id=" & G.Rows(i).Cells(GC.Id).Value.ToString) = 0 Then
            ...
            G.Rows(i).Cells(GC.Commission).Value = Val(G.Rows(i).Cells(GC.Commission).Value)
            ...
            G.Rows(i).Cells(GC.CustVal).Value = Val(G.Rows(i).Cells(GC.CustVal).Value)
            G.Rows(i).Cells(GC.CompVal).Value = Val(G.Rows(i).Cells(GC.Value).Value) - Val(G.Rows(i).Cells(GC.CustVal).Value)
    ...
    Sub ClearRow(ByVal i As Integer)
        ...
        G.Rows(i).Cells(GC.CustVal).Value = Nothing
        G.Rows(i).Cells(GC.CompVal).Value = Nothing
        ItemBal.Clear()
    End Sub
    Private Sub RdoCash_Checked(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles RdoCash.Checked, RdoVisa.Checked, RdoCashVisa.Checked, RdoFuture.Checked, RdoCashFuture.Checked, RdoEmployees.Checked
        ...
        Try
            If (RdoCash.IsChecked OrElse RdoCashFuture.IsChecked OrElse RdoCashVisa.IsChecked) AndAlso (TestPurchaseAndReturn() OrElse TestSalesAndReturn() OrElse TestExportAndReturn()) Then
                If Not Md.MyProjectType = ProjectType.X23 Then
    ...
    Private Sub GridCalcRow(ByVal sender As Object, ByVal e As Forms.DataGridViewCellEventArgs)
        ...
        Try
            ...
            ElseIf G.Columns(e.ColumnIndex).Name = GC.ItemSerialNo AndAlso Not G.Rows(e.RowIndex).Cells(GC.ItemSerialNo).Value Is Nothing AndAlso Not G.Rows(e.RowIndex).Cells(GC.ItemSerialNo).Value = "" Then
                ...
                If dt.Rows.Count > 0 Then
                    If Md.MyProjectType = ProjectType.X28 AndAlso TestPurchaseOnly() Then
    ...
    Private Sub StoreId_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles StoreId.KeyUp
        ...
        If TestConsumablesAndReturn() AndAlso Not Md.MyProjectType = ProjectType.X8 Then
            str &= " and Flag=1 "
        End If
    ...
    Private Sub ToId_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles ToId.KeyUp
        ...
        ElseIf ((TestPurchaseAndReturn() OrElse TestImportAndReturn()) AndAlso Not ReservToId.IsChecked) OrElse ((TestSalesAndReturn() OrElse TestExportAndReturn()) AndAlso ReservToId.IsChecked) Then
            tbl = "Suppliers"
            Title = "الموردين"
            If bm.ShowHelp(Title, ToId, ToName, e, "select cast(Id as varchar(100)) Id,Name from " & tbl, IIf(Md.MyProjectType = ProjectType.X23 OrElse Md.MyProjectType = ProjectType.X8, tbl, "")) Then
                ToId_LostFocus(sender, Nothing)
            End If
        ...
        ElseIf TestConsumablesAndReturn() Then
            If bm.ShowHelpCases(ToId, ToName, e, True, IIf(Md.MyProjectType = ProjectType.X11, False, True)) Then
                ToId_LostFocus(sender, Nothing)
            End If
        End If
    End Sub
    ...
    Private Sub ToId_LostFocus(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles ToId.LostFocus
        ...
        ElseIf TestConsumablesAndReturn() Then
            ...
            If dt.Rows.Count > 0 Then
                ToId.ToolTip = Resources.Item("Id") & ": " & ToId.Text & vbCrLf & Resources.Item("Name") & ": " & ToName.Text & vbCrLf & Resources.Item("HomePhone") & ": " & dt.Rows(0)("HomePhone").ToString & vbCrLf & Resources.Item("Mobile") & ": " & dt.Rows(0)("Mobile").ToString & vbCrLf & Resources.Item("Notes") & ": " & dt.Rows(0)("Notes").ToString
        ...
        If ((TestSalesAndReturn() OrElse TestExportAndReturn()) AndAlso Not ReservToId.IsChecked) OrElse ((TestPurchaseAndReturn()) AndAlso ReservToId.IsChecked) Then
            ...
            End If
        End If
        If TestImportAndReturn() OrElse TestExportAndReturn() OrElse (Md.MyProjectType = ProjectType.X8 AndAlso TestPurchaseAndReturn()) Then
            RdoCash.IsChecked = True
            RdoFuture.IsChecked = True
        End If
        ToId.ToolTip = s
    ...
    Private Sub CashierId_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles CashierId.KeyUp
        bm.ShowHelp("Employees", CashierId, CashierName, e, "select cast(Id as varchar(100)) Id," & Resources.Item("CboName") & " Name from Employees where Doctor=0 and Stopped=0" & IIf(TestConsumablesAndReturn() AndAlso Md.MyProjectType = ProjectType.X11, " and Nurse=1", " and Cashier=1"))
    End Sub
    ...
    Private Sub CashierId_LostFocus(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles CashierId.LostFocus
        bm.LostFocus(CashierId, CashierName, "select " & Resources.Item("CboName") & " Name from Employees where Doctor=0 and Id=" & CashierId.Text.Trim() & " and Stopped=0 " & IIf(TestConsumablesAndReturn() AndAlso Md.MyProjectType = ProjectType.X11, " and Nurse=1", " and Cashier=1"))
    End Sub
    ...
    Sub FillControls()
        ...
        Dim dt As DataTable = bm.ExecuteAdapter("select SD.* ,It.Name It_Name,dbo.GetGroupName(It.GroupId)GroupName,dbo.GetTypeName(It.GroupId,It.TypeId)TypeName from SalesDetails SD left join Items It on(SD.ItemId=It.Id) where SD.StoreId=" & StoreId.Text & " and SD.InvoiceNo=" & InvoiceNo.Text & " and SD.Flag=" & Flag)
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            ...
            G.Rows(i).Cells(GC.Commission).Value = dt.Rows(i)("Commission").ToString
            ...
            G.Rows(i).Cells(GC.CustVal).Value = dt.Rows(i)("CustVal").ToString
            G.Rows(i).Cells(GC.CompVal).Value = dt.Rows(i)("CompVal").ToString
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles btnPrint.Click, btnPrint2.Click, btnPrint3.Click, btnPrint4.Click, btnPrint5.Click, btnPrintImage.Click, btnReceiveSave.Click, btnPrintSafe.Click
        If Md.Manager OrElse Md.MyProjectType = ProjectType.X26 Then DontClear = True
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If AccNo.Visibility = Windows.Visibility.Visible AndAlso Val(AccNo.Text) = 0 Then
            ...
            Return
        End If
        If ToId.Visibility = Windows.Visibility.Visible AndAlso ToId.Text.Trim = "" AndAlso Flag <> FlagState.أمر_شراء Then
        ...
        If Md.MyProjectType = ProjectType.X17 AndAlso DocNo.Text.Trim = "" Then
            Select Case Flag
                Case FlagState.المبيعات, FlagState.مردودات_مشتريات, FlagState.مردودات_مشتريات_خارجية, FlagState.مردودات_الاستيراد
                    DocNo.Text = Val(bm.ExecuteScalar("select dbo.GetSalesNewSerial(" & StoreId.Text & "," & 3 & ")"))
            End Select
        End If
        ...
        If btnSave.IsEnabled Then
            ...
            If Not bm.SaveGrid(G, "SalesDetails", New String() {"Flag", "StoreId", "InvoiceNo"}, New String() {Flag, StoreId.Text, InvoiceNo.Text}, New String() {"SalesInvoiceNo", "Barcode", "ItemSerialNo", "ItemId", "ItemName", "ExpDate", "ExpireDate", "Color", "Size", "UnitId", "UnitQty", "Qty", "Qty2", "Qty3", "ReceivedQty", "Price", "Commission", "QtySub", "PriceSub", "ItemDiscountPerc", "ItemDiscount", "Value", "IsPrinted", "SalesPrice", "FlagType", "SerialNo", "IsDelivered", "SalesManId", "ProductionOrderFlag", "ProductionOrderNo", "SD_Notes", "UnitsWeightId", "UnitsWeightQty", "PreQty", "SalesPriceTypeId", "AvgCost", "SalesReturnStateId", "SalesReturnStateReason", "CustVal", "CompVal"}, New String() {GC.SalesInvoiceNo, GC.Barcode, GC.ItemSerialNo, GC.Id, GC.Name, GC.ExpDate, GC.ExpireDate, GC.Color, GC.Size, GC.UnitId, GC.UnitQty, GC.Qty, GC.Qty2, GC.Qty3, GC.ReceivedQty, GC.Price, GC.Commission, GC.QtySub, GC.PriceSub, GC.ItemDiscountPerc, GC.ItemDiscount, GC.Value, GC.IsPrinted, GC.SalesPrice, GC.FlagType, GC.SerialNo, GC.IsDelivered, GC.SalesManId, GC.ProductionOrderFlag, GC.ProductionOrderNo, GC.SD_Notes, GC.UnitsWeightId, GC.UnitsWeightQty, GC.PreQty, GC.SalesPriceTypeId, GC.AvgCost, GC.SalesReturnStateId, GC.SalesReturnStateReason, GC.CustVal, GC.CompVal}, New VariantType() {VariantType.Integer, VariantType.String, VariantType.String, VariantType.Integer, VariantType.String, VariantType.String, VariantType.Date, VariantType.Integer, VariantType.Integer, VariantType.Integer, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Decimal, VariantType.Integer, VariantType.Decimal, VariantType.Integer, VariantType.String, VariantType.Integer, VariantType.Integer, VariantType.Integer, VariantType.Integer, VariantType.String, VariantType.Integer, VariantType.Decimal, VariantType.Decimal, VariantType.Integer, VariantType.Decimal, VariantType.Integer, VariantType.String, VariantType.Decimal, VariantType.Decimal}, New String() {GC.Id}, "Line", "Line") Then Return
            ...
            If Md.MyProjectType = ProjectType.X16 AndAlso TestSalesAndReturn() Then
...
Print:
        ...
        If sender Is btnPrint OrElse sender Is btnPrint2 OrElse sender Is btnPrint3 OrElse sender Is btnPrint4 OrElse sender Is btnPrint5 OrElse sender Is btnPrintImage OrElse sender Is btnPrintSafe Then
            PrintPone(sender, 0)
            If Md.MyProjectType = ProjectType.X17 AndAlso sender Is btnPrint AndAlso Flag = FlagState.المبيعات Then
    ...
    Sub ClearControls()
        Try
            ...
            If Md.MyProjectType = ProjectType.X24 AndAlso TestSalesAndReturn() Then
            ...
            If SalesTypeId.Items.Count > 0 Then
                ...
                If Md.MyProjectType = ProjectType.X24 AndAlso TestSalesOnly() Then
            ...
            If Md.MyProjectType <> ProjectType.X11 AndAlso Md.MyProjectType <> ProjectType.X1 AndAlso Md.MyProjectType <> ProjectType.X2 AndAlso Md.MyProjectType <> ProjectType.X17 Then CashierId.Text = Md.UserName
            ...
            If TestPurchaseAndReturn() Then
                If Md.MyProjectType = ProjectType.X26 AndAlso TestOuterPurchaseAndReturn() Then
                Else
            ...
            If Md.MyProjectType = ProjectType.X14 AndAlso TestSalesOnly() Then
                Temp.Visibility = Windows.Visibility.Visible
                Temp.Content = "فاتورة حجز"
            ElseIf Md.MyProjectType = ProjectType.X14 AndAlso TestPurchaseOnly() Then
                Temp.Visibility = Windows.Visibility.Visible
            ...
            If Not Md.Manager Then
                ...
                If Not Md.MyProjectType = ProjectType.X17 Then CashierId.Text = Md.UserName
                CashierId_LostFocus(Nothing, Nothing)
            Else
        ...
        If TestSalesAndReturn() AndAlso Md.MyProjectType = ProjectType.X17 Then
            DeliveryDate.SelectedDate = DayDate.SelectedDate
        End If


        'If Md.MyProjectType = ProjectType.Clothes AndAlso TestPurchaseOnly() AndAlso Not Md.Manager Then Temp.IsChecked = True
    ...
    Private Sub InvoiceNo_KeyUp(sender As Object, e As KeyEventArgs) Handles InvoiceNo.KeyUp
        If TestImportAndReturn() Then
            If bm.ShowHelpMultiColumns("Header", InvoiceNo, InvoiceNo, e, "select cast(InvoiceNo as nvarchar(100))'رقم الفاتورة',cast(ToId as nvarchar(100))'كود المورد',dbo.GetSupplierName(ToId)'اسم المورد',dbo.GetOrderTypes(OrderTypeId)'رقم الطلبية',DocNo'رقم عقد المورد',dbo.ToStrDate(DayDate)'التاريخ',DocNo 'رقم المستند',Notes 'الملاحظات' from SalesMaster where StoreId=" & Val(StoreId.Text) & " and Flag=" & Flag.ToString) Then
                'InvoiceNo.Text = bm.SelectedRow(0)
                InvoiceNo_Leave(Nothing, Nothing)
            End If
        ElseIf TestPurchaseAndReturn() Then
            If bm.ShowHelpMultiColumns("Header", InvoiceNo, InvoiceNo, e, "select cast(InvoiceNo as nvarchar(100))'رقم الفاتورة',cast(ToId as nvarchar(100))'كود المورد',dbo.GetSupplierName(ToId)'رقم عقد المورد',dbo.ToStrDate(DayDate)'التاريخ',DocNo 'رقم المستند',Notes 'الملاحظات' from SalesMaster where StoreId=" & Val(StoreId.Text) & " and Flag=" & Flag.ToString) Then
                'InvoiceNo.Text = bm.SelectedRow(0)
                InvoiceNo_Leave(Nothing, Nothing)
            End If
        ElseIf TestSalesAndReturn() OrElse TestExportAndReturn() Then
            If bm.ShowHelpMultiColumns(CType(Parent, Page).Title, InvoiceNo, InvoiceNo, e, "select cast(InvoiceNo as nvarchar(100))'رقم الفاتورة',cast(ToId as nvarchar(100))'كود العميل',dbo.GetCustomerName(ToId)'اسم العميل',dbo.ToStrDate(DayDate)'التاريخ',DocNo 'رقم المستند',Notes 'الملاحظات' from SalesMaster where StoreId=" & Val(StoreId.Text) & " and Flag=" & Flag.ToString) Then
                'InvoiceNo.Text = bm.SelectedRow(0)
                InvoiceNo_Leave(Nothing, Nothing)
            End If
        End If
    ...
    Private Sub PrintPone(ByVal sender As System.Object, ByVal NewItemsOnly As Integer)
        ...
        If TestSalesAndReturn() OrElse TestPurchaseAndReturn() Then
            Select Case Md.MyProjectType
                Case ProjectType.X17, ProjectType.X29
                    MyBal = Val(bm.ExecuteScalar("select dbo.Bal0(AccNo,Id,getdate(),0,0,0) from " & IIf(ReservToId.IsChecked, "Suppliers", "Customers") & " where Id=" & ToId.Text))
            End Select
        End If
        ...
        If NewItemsOnly = 0 Then
            ...
            Select Case Md.MyProjectType
                ...
                Case Else
                    ...
                    ElseIf Md.MyProjectType = ProjectType.Optics Then
                        ...
                        If bm.GridSumColumn(G, GC.ItemDiscount) = 0 Then
                            rpt.Rpt = "SalesPone_O2.rpt"
                        End If
                    ElseIf Md.MyProjectType = ProjectType.X8 Then
            ...
            If sender Is btnPrint3 AndAlso Md.MyProjectType = ProjectType.X14 Then rpt.Rpt = "SalesPone33.rpt"
            If sender Is btnPrint3 AndAlso Md.MyProjectType = ProjectType.X15 Then rpt.Rpt = "SalesPone3_H.rpt"
            If sender Is btnPrint3 AndAlso Md.MyProjectType = ProjectType.Optics Then rpt.Rpt = "SalesPone3_O.rpt"

            If sender Is btnPrint4 Then rpt.Rpt = IIf(Md.MyProjectType = ProjectType.X17, "SalesPone4_N.rpt", "SalesPone4.rpt")
            If sender Is btnPrintSafe Then rpt.Rpt = "SalesPonePrintSafe.rpt"

            If sender Is btnPrint2 Then
                Dim i As Integer = 1
                rpt.Rpt = IIf(Md.MyProjectType = ProjectType.X17, "SalesPone2_N.rpt", "SalesPone2.rpt")
                rpt.Rpt = IIf(Md.MyProjectType = ProjectType.X29, "SalesPone2_AZ.rpt", rpt.Rpt)
            ...
            If sender Is btnPrint5 Then
                ...
                ElseIf Md.MyProjectType = ProjectType.X29 Then
            ...
            If Md.MyProjectType = ProjectType.X17 AndAlso sender Is btnPrint Then
                rpt.ShowDialog()
            Else
                rpt.Show()
            End If
    ...
    Private Sub RdoGrouping_Checked(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles RdoGrouping.Checked, RdoSearch.Checked
        Try
            If RdoGrouping.IsChecked Then
                ...
                btnItemsSearch0.Visibility = Visibility.Visible
            ...
            ElseIf RdoSearch.IsChecked Then
                ...
                btnItemsSearch0.Visibility = Visibility.Hidden
    ...
    Sub CalcTotalEnd() Handles Per1.TextChanged, Per2.TextChanged, Per3.TextChanged, Per4.TextChanged, Val1.LostFocus, Val2.LostFocus, Val3.LostFocus, Val4.LostFocus, AccType1.SelectionChanged, AccType2.SelectionChanged, AccType3.SelectionChanged, AccType4.SelectionChanged, Shipping.LostFocus, Freight.LostFocus, CustomClearance.LostFocus
        ...
        If Md.MyProjectType.X26 And TestSalesAndReturn() Then
        ...
        Try
            ...
            If G.CurrentCell.ColumnIndex = G.Columns(GC.SalesInvoiceNo).Index Then
                ...
                If bm.ShowHelpGridMultiColumns("Invoices", G.CurrentRow.Cells(GC.SalesInvoiceNo), G.CurrentRow.Cells(GC.SalesInvoiceNo), e, "select distinct M.InvoiceNo 'الفاتورة',dbo.ToStrDate(M.DayDate) 'التاريخ',D.ItemId 'كود الصنف',D.ItemName 'اسم الصنف',D.Qty 'الكمية',D.Price -D.ItemDiscount 'السعر بعد الخصم',D.Value 'القيمة' from SalesMaster M left join SalesDetails D on(M.Flag=D.Flag and M.StoreId=D.StoreId and M.InvoiceNo=D.InvoiceNo) where M.StoreId=" & StoreId.Text & " and M.Flag=" & MainFlag() & " and M.ToId=" & Val(ToId.Text) & " and M.OrderTypeId=" & Val(OrderTypeId.Text) & " and (D.ItemId=" & Val(G.Rows(G.CurrentCell.RowIndex).Cells(GC.Id).Value) & " or " & Val(G.Rows(G.CurrentCell.RowIndex).Cells(GC.Id).Value) & "=0)") Then
                    ...
                    GridCalcRow(sender, New Forms.DataGridViewCellEventArgs(G.Columns(GC.Price).Index, G.CurrentCell.RowIndex))
    ...
    Private Sub HideAcc4()
        Tab4.Visibility = Windows.Visibility.Collapsed
    End Sub

    Private Sub HideAcc5()
        Tab5.Visibility = Windows.Visibility.Collapsed
    End Sub
    ...
    Private Sub LoadItemPrice(i As Integer)
        ...
        G.Rows(i).Cells(GC.Commission).Value = dt.Rows(0)("Commission")
    ...
    Private Sub LoadVisibility()
        ...
        AgrementNo.Visibility = Visibility.Hidden
        lblAgrementNo.Visibility = Visibility.Hidden
        ...
        If Md.MyProjectType = ProjectType.X24 AndAlso TestSalesAndReturn() Then
            btnAddCustomer.Visibility = Windows.Visibility.Visible
        End If
        ...
        If TestPurchaseAndReturn() Then
            ...
            If Md.MyProjectType = ProjectType.X8 Then
                lblDeliveryDate.Visibility = Windows.Visibility.Visible
                DeliveryDate.Visibility = Windows.Visibility.Visible
                lblDeliveryDate.Content = "ت فاتورة المورد"
            End If

            Exchange.Visibility = Visibility.Visible
            lblExchange.Visibility = Visibility.Visible
        ...
        ElseIf TestSalesAndReturn() OrElse TestExportAndReturn() Then
            ...
            If Md.MyProjectType = ProjectType.X17 OrElse Md.MyProjectType = ProjectType.X15 OrElse Md.MyProjectType = ProjectType.X26 Then
                lblDeliveryDate.Visibility = Windows.Visibility.Visible
                DeliveryDate.Visibility = Windows.Visibility.Visible
                If Md.MyProjectType = ProjectType.X17 Then
                    lblDeliveryDate.Content = "تاريخ التحميل"
                ElseIf Md.MyProjectType = ProjectType.X26 Then
            ...
            If Not (Md.MyProjectType = ProjectType.X4 OrElse Md.MyProjectType = ProjectType.X5 OrElse Md.MyProjectType = ProjectType.X23 OrElse Md.MyProjectType = ProjectType.X25 OrElse Md.MyProjectType = ProjectType.X17) Then
            ...
            If Md.MyProjectType = ProjectType.Optics Then
                Tab5.Visibility = Windows.Visibility.Visible
                PanelAcc.SelectedItem = Tab5
            End If
            Exchange.Visibility = Visibility.Visible
            lblExchange.Visibility = Visibility.Visible

            If TestSalesOnly() Then
                AgrementNo.Visibility = Visibility.Visible
                lblAgrementNo.Visibility = Visibility.Visible
            End If
        ...
        If Md.MyProjectType = ProjectType.X14 Then
            ...
            'PanelTypes.Visibility = Windows.Visibility.Hidden
            'PanelItems.Visibility = Windows.Visibility.Hidden
            'HelpGD.Visibility = Windows.Visibility.Hidden
            'WP.Visibility = Windows.Visibility.Hidden
        ...
        If Not Flag = FlagState.مشتريات OrElse Not Md.MyProjectType = ProjectType.X11 Then
        ...
        If (Md.MyProjectType = ProjectType.X24 OrElse Md.MyProjectType = ProjectType.Optics) AndAlso TestSalesOnly() AndAlso Flag <> FlagState.عرض_أسعار AndAlso Flag <> FlagState.أمر_توريد Then
            lblSalesTypeId.Visibility = Windows.Visibility.Visible
            SalesTypeId.Visibility = Windows.Visibility.Visible
        End If


        If Not Md.Manager AndAlso Not Md.MyProjectType = ProjectType.X11 Then
        ...
        If Md.MyProjectType = ProjectType.X13 OrElse Flag = FlagState.عرض_أسعار OrElse Flag = FlagState.أمر_توريد OrElse Flag = FlagState.أمر_شراء OrElse Flag = FlagState.عرض_أسعار_مورد Then
        ...
        If (Md.MyProjectType = ProjectType.X17 OrElse Md.MyProjectType = ProjectType.X15) AndAlso Flag <> FlagState.مشتريات AndAlso Flag <> FlagState.مردودات_مشتريات AndAlso Flag <> FlagState.مشتريات_خارجية AndAlso Flag <> FlagState.مردودات_مشتريات_خارجية AndAlso Flag <> FlagState.الاستيراد AndAlso Flag <> FlagState.مردودات_الاستيراد Then
            DocNo.Visibility = Visibility.Hidden
            lblDocNo.Visibility = Visibility.Hidden
        End If
        ...
        bm.TestSecurity(Me, {btnSave, DayDate}, {btnDelete}, {btnFirst, btnNext, btnPrevios, btnLast, InvoiceNo}, {btnPrint, btnPrint2, btnPrint3, btnPrint4, btnPrint5, btnPrintImage, btnPrintSafe})
    ...
    Private Sub GetItemNameAndBal(i As Integer, Id As String, ItemSerialNo As String)
        ...
        Dim dt As DataTable = bm.ExecuteAdapter("Select dbo.GetStoreItemBal('" & StoreId.Text & "','" & Id & "','" & ItemSerialNo & "',0,'" & G.Rows(i).Cells(GC.Size).Value & "',getdate())Bal,*,dbo.GetGroupName(GroupId)GroupName,dbo.GetTypeName(GroupId,TypeId)TypeName From Items_View  where " & str & " Id='" & Id & "' " & ItemWhere())
        ...
        If TestSalesAndReturn() AndAlso Val(G.Rows(i).Cells(GC.ItemDiscount).Value) = 0 Then
            G.Rows(i).Cells(GC.ItemDiscountPerc).Value = dr(0)("ItemDiscountPerc")
            G.Rows(i).Cells(GC.ItemDiscount).Value = dr(0)("ItemDiscountValue")
        End If
    ...
    Private Sub DocNo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles DocNo.KeyUp
        If Not (Md.MyProjectType = ProjectType.X17 AndAlso TestPurchaseOnly()) Then Return
    ...
    Private Sub btnGenerateSalesInvoiceFromQuotation_Click(sender As Object, e As RoutedEventArgs) Handles btnGenerateSalesInvoiceFromQuotation.Click
        ...
        Else
            If bm.ShowDeleteMSG("هل أنت متأكد من إنشاء فاتورة مبيعات؟") Then
                ...
                btnSave_Click(sender, e)
                DontClear = False
                Dim x As Integer = bm.ExecuteScalar("GenerateSalesInvoiceFromQuotation", {"StoreId", "InvoiceNo"}, {Val(StoreId.Text), Val(InvoiceNo.Text)})
                bm.ShowMSG("تم إنشاء فاتورة مبيعات برقم " & x)
                OpenSales("المبيعات", x, Sales.FlagState.المبيعات)
    ...
    Sub OpenSales(MyTitle As String, MyInvoiceNo As Integer, MyFlag As Integer)

        Dim frm As New MyWindow With {.Title = MyTitle, .WindowState = WindowState.Maximized}
        Dim c As New Sales With {.MyInvoiceNo = MyInvoiceNo, .MyStoreId = Val(StoreId.Text), .Flag = MyFlag}
        'c.Sales_Loaded(Nothing, Nothing)
        bm.SetMySecurityType(frm, 549)
        frm.Content = c
        frm.Hide()
        frm.ShowDialog()
    ...
    Private Sub txtName2_TextChanged(sender As Object, e As TextChangedEventArgs) Handles txtName2.TextChanged
        Load_dv_items()
    End Sub

    Dim MyBath As String = ""

    Private Sub btnItemsSearch0_Click(sender As Object, e As RoutedEventArgs) Handles btnItemsSearch0.Click
        If WItems.Tag Is Nothing Then Return
        Try
            Dim frm As New ItemsSearch With {.StoreId = Val(StoreId.Text), .GroupId = WTypes.Tag.ToString, .TypeId = WItems.Tag.ToString, .GroupName = TabTypes.Header.ToString.Substring(10), .TypeName = TabItems.Header.ToString.Substring(10)}
            frm.ShowDialog()
            'If frm.Id > 0 Then
            '    AddItem(frm.Id)
            'End If
            For i As Integer = 0 To frm.Ids.Count - 1
                AddItem(frm.Ids(i))
            Next
        Catch ex As Exception
        End Try
    ...
    Private Sub Cyl_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles CylR.KeyUp, CylL.KeyUp, CylR2.KeyUp, CylL2.KeyUp
        bm.ShowHelp(sender.Name, sender, sender, e, "select distinct CLY Id,CLY Name from Items order by CLY")
    End Sub


    Private Sub Sph_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles SphR.KeyUp, SphL.KeyUp, SphR2.KeyUp, SphL2.KeyUp
        bm.ShowHelp(sender.Name, sender, sender, e, "select distinct SPH Id,SPH Name from Items order by SPH")
    End Sub


    Private Sub txt_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles AxisR.KeyUp, AxisL.KeyUp, AxisR2.KeyUp, AxisL2.KeyUp
        bm.ShowHelp(sender.Name, sender, sender, e, "select distinct " & sender.Name & " Id," & sender.Name & " Name from " & TableName & " where " & sender.Name & "<>''")
    End Sub

    Private Sub btnWhatsApp_Click(sender As Object, e As RoutedEventArgs) Handles btnWhatsApp.Click
        Dim rpt As New ReportViewer
        rpt.paraname = New String() {"@FromDate", "@ToDate", "@Shift", "@Flag", "@StoreId", "@FromInvoiceNo", "@ToInvoiceNo", "@InvoiceNo", "@NewItemsOnly", "@RPTFlag1", "@RPTFlag2", "@PrintingGroupId", "Header", "Remaining", "Payed", "Bal"}
        rpt.paravalue = New String() {DayDate.SelectedDate, DayDate.SelectedDate, Val(Shift.SelectedValue), Flag, StoreId.Text, InvoiceNo.Text, InvoiceNo.Text, InvoiceNo.Text, 0, 0, 0, 0, CType(Parent, Page).Title, Val(Remaining.Text), Val(Payed.Text), 0}
        rpt.Rpt = "SalesPone_O.rpt"
        If bm.GridSumColumn(G, GC.ItemDiscount) = 0 Then
            rpt.Rpt = "SalesPone_O2.rpt"
        End If
        ...
        Dim fileName As String = bm.GetNewTempName(ToId.Text & ".pdf")
        rpt.ReportDoc.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, fileName)
        Process.Start("explorer.exe", "/select," & fileName)

        Process.Start("whatsapp://send?phone=2" & bm.ExecuteScalar("select Mobile from customers where Id=" & ToId.Text) & "&Text=" & System.Net.WebUtility.UrlEncode(bm.ExecuteScalar("select WhatsAppNotes from Stores where Id=" & Val(StoreId.Text))))

        'Dim zero = IntPtr.Zero
        'zero = FindWindow(Nothing, "WhatsApp")
