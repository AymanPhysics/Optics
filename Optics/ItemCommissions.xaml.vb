...
Public Class ItemCommissions
    ...
    Private Sub LoadTypes(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Try
            ...
            If All.IsChecked Then
                Dim str As String = "select It.Id from Items It where It.GroupId=" & xx.Tag & " and IsStopped=0 group by It.Id"
    ...
    Private Sub LoadItems(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Try
            ...
            If All.IsChecked Then
                Dim dtdt As DataTable = bm.ExecuteAdapter("select Id from Items where GroupId=" & WTypes.Tag.ToString & " and TypeId=" & xx.Tag & " and IsStopped=0 ")
                For i As Integer = 0 To dtdt.Rows.Count - 1
                    AddItem(dtdt.Rows(i)(0))
                Next
            End If
...
Br:
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        For i As Integer = 0 To G.Rows.Count - 1
            Try
                ...
                If Not bm.ExecuteScalar("Update Items set Commission='" & Val(G.Rows(i).Cells(GC.Commission).Value) & "' where Id=" & Val(G.Rows(i).Cells(GC.Id).Value)) Then
                    bm.ShowMSG("لم يتم حفظ الصنف رقم " & G.Rows(i).Cells(GC.Id).Value)
                End If
            Catch ex As Exception
            End Try
        Next
    ...
    Private Sub btnSave_Copy_Click(sender As Object, e As RoutedEventArgs) Handles btnSave_Copy.Click
        ...
        For i As Integer = 0 To G.Rows.Count - 1
            G.Rows(i).Cells(GC.Commission).Value = G.Rows(i).Cells(GC.SalesPrice).Value * Val(Value.Text) / 100
        Next
    End Sub

    Private Sub btnSave_Copy1_Click(sender As Object, e As RoutedEventArgs) Handles btnSave_Copy1.Click
        For i As Integer = 0 To G.Rows.Count - 1
            G.Rows(i).Cells(GC.Commission).Value = Val(Value.Text)
        Next
    End Sub
