...
Public Class Inventory
    ...
    Private Sub Sales_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        bm.TestSecurity(Me, {btnSave, btnInventory}, {btnDelete}, {btnFirst, btnNext, btnPrevios, btnLast}, {btnPrint, btnPrint1, btnPrint2})
        ...
        bm.Fields = New String() {MainId, SubId, "DayDate", "Notes", "DocNo", "InInvoiceNo", "OutInvoiceNo", "TotalCount"}
        bm.control = New Control() {StoreId, InvoiceNo, DayDate, Notes, DocNo, InInvNo, OutInvNo, TotalCount}
        ...
        If Not Md.Manager Then btnInventory.Visibility = Visibility.Hidden
...
Br:
            ...
            If xx >= 0 AndAlso xx <> i Then
                PlaySoundError()
                bm.ShowMSG("تم تكرار هذا الصنف بالسطر رقم " + (xx + 1).ToString)
                ClearRow(i)
                G.CurrentCell = G.Rows(xx).Cells(GC.Name)
                Return -1
            End If
            ...
            If dr.Length = 0 Then
                If Not G.Rows(i).Cells(GC.Id).Value Is Nothing Or G.Rows(i).Cells(GC.Id).Value <> "" Then
                    PlaySoundError()
        ...
        Try
            If G.Columns(e.ColumnIndex).Name = GC.Barcode AndAlso Not G.Rows(e.RowIndex).Cells(GC.Barcode).Value Is Nothing Then
                ...
                If Not G.Rows(e.RowIndex).Cells(GC.Barcode).Value = Nothing Then
                    ...
                    G.Rows(e.RowIndex).Cells(GC.Id).Value = bm.ExecuteScalar("select Id from Items where IsStopped=0 and Barcode='" & G.Rows(e.RowIndex).Cells(GC.Barcode).Value.ToString & "'")
    ...
    Private Sub CalcTotal()
        ...
        Try
            LopCalc = True
            TotalCount.Text = bm.GridSumColumn(G, GC.RealBal)
            LopCalc = False
        Catch ex As Exception
        End Try
    End Sub
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        ElseIf sender Is btnInventory Then
            'DontClear = True
            If bm.ShowDeleteMSG("هل أنت متأكد من إجراء التسوية؟") Then
    ...
    Private Sub btnPrint_Copy_Click(sender As Object, e As RoutedEventArgs) Handles btnInventory.Click, btnPrint.Click, btnPrint1.Click, btnPrint2.Click
    ...
    Private Sub Barcode_KeyDown(sender As Object, e As KeyEventArgs) Handles Barcode.KeyDown
        If e.Key = Key.Tab Then
            ...
            If Not IsExists Then
                Dim id As String = bm.ExecuteScalar("select Id from Items where Barcode='" & Barcode.Text & "'")
                If id = "" Then
                    PlaySoundError()
                    bm.ShowMSG("هذا الصنف غير موجود")
                    Barcode.Clear()
    ...
    Sub PlaySound()
        Dim soundPlayer As New System.Media.SoundPlayer("c:\Windows\Media\Windows Error.wav")
        soundPlayer.Play()
    End Sub

    Sub PlaySoundError()
        Dim soundPlayer As New System.Media.SoundPlayer("c:\Windows\Media\Alarm01.wav")
        soundPlayer.Play()
    End Sub
