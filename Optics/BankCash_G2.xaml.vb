...
Public Class BankCash_G2
    ...
    Private Sub LoadWFH(WFH As Forms.Integration.WindowsFormsHost, G As MyGrid)
        ...
        If Md.MyProjectType = ProjectType.X17 OrElse Md.MyProjectType = ProjectType.X15 OrElse Md.MyProjectType = ProjectType.Optics Then
            ...
            PurchaseAccName.Visibility = Windows.Visibility.Visible
            ImportMessageName.Visibility = Windows.Visibility.Visible
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        For i As Integer = 0 To G1.Rows.Count - 1
            ...
            If Md.MyProjectType = ProjectType.X17 AndAlso G1.Columns(GC.CostTypeId).Visible AndAlso Val(G1.Rows(i).Cells(GC.LinkFile).Value) = 9 AndAlso Val(G1.Rows(i).Cells(GC.CostTypeId).Value) = 0 Then
        ...
        For i As Integer = 0 To G2.Rows.Count - 1
            ...
            If Md.ShowBankCash_GAccNo_Not_LinkFile AndAlso Val(G2.Rows(i).Cells(GC.MainAccNo).Value) = 0 Then
                ...
                Return
            ElseIf Not G2.Rows(i).Cells(GC.SubAccNo).ReadOnly AndAlso Val(G2.Rows(i).Cells(GC.SubAccNo).Value) = 0 Then
                bm.ShowMSG("برجاء تحديد الفرعى بالسطر " & (i + 1).ToString)
                G2.Focus()
                G2.CurrentCell = G2.Rows(i).Cells(GC.SubAccNo)
                Return
            End If
        Next
            ...
            ElseIf G.Columns(e.ColumnIndex).Name = GC.SubAccNo Then
                ...
                Else
                    ...
                    If dt.Rows.Count > 0 Then
                        ...
                        If Val(G.Rows(e.RowIndex).Cells(GC.LinkFile).Value) = 1 AndAlso Val(G.Rows(e.RowIndex).Cells(GC.SubAccNo).Value) > 0 AndAlso G.Rows(e.RowIndex).Cells(GC.MainValue).Value Is Nothing Then
                            G.Rows(e.RowIndex).Cells(GC.MainValue).Value = bm.ExecuteScalar("select dbo.Bal0Link('1','" & Val(G.Rows(e.RowIndex).Cells(GC.SubAccNo).Value) & "','" & bm.ToStrDate(bm.MyGetDate()) & "',0)")
                            GridCalcRow(G, New DataGridViewCellEventArgs(G.Columns(GC.MainValue).Index, e.RowIndex))
                        End If
                    Else
                            G.Rows(e.RowIndex).Cells(GC.SubAccNo).Value = ""
                        G.Rows(e.RowIndex).Cells(GC.SubAccName).Value = ""
    ...
    Sub ClearControls()
        ...
        If Md.MyProjectType = ProjectType.X25 AndAlso Not DayDate.SelectedDate Is Nothing Then
    ...
    Private Sub SetGCExchange(G As MyGrid)
        If Md.MyProjectType = ProjectType.X25 Then Return
        Try
            If Flag = 2 Then
