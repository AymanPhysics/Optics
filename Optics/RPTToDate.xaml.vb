    ...
    Public Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles Button2.Click
        If Flag = 1 Then
            If bm.ShowDeleteMSG("سيتم مسح كافة البيانات حتى تاريخ " & bm.ToStrDate(ToDate.SelectedDate) & " .. استمرار؟") Then
                If bm.ExecuteNonQuery("exec bk", False) AndAlso bm.ExecuteNonQuery("delete Reservations where DayDate<='" & bm.ToStrDate(ToDate.SelectedDate) & "' delete Services where DayDate<='" & bm.ToStrDate(ToDate.SelectedDate) & "' delete EmpCloseShift where DayDate<='" & bm.ToStrDate(ToDate.SelectedDate) & "' delete EmpOutCome where DayDate<='" & bm.ToStrDate(ToDate.SelectedDate) & "'  exec SetBalances '" & bm.ToStrDate(ToDate.SelectedDate) & "'") Then
