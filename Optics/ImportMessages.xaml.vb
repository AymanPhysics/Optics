...
Public Class ImportMessages
    ...
    Private Sub LabTestItems_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        bm.TestSecurity(Me, {btnSave}, {btnDelete}, {btnFirst, btnNext, btnPrevios, btnLast}, {btnPrint, btnPrint2, btnPrint2Images, btnPrint3, btnPrint3Images, btnPrintImages})
        ...
        LoadWFH1()
        LoadWFH2()
        bm.Fields = New String() {MainId, SubId, "AccNo", "DayDate", "IsDelivered", "DeliveredDate", "StoreId", "ShipperId", "ShippingLineId", "ShippingCompanyId", "ContainerSizeId", "ContainerPrice", "ContainerSizeCurrencyId", "PolisaData", "Perc", "Cost_InvoiceNo", "Cost_MessageId", "Cost_OrderTypeId", "Cost_Administrative", "Bal0", "val5"}
        bm.control = New Control() {OrderTypeId, txtID, MainAccNo, DayDate, IsDelivered, DeliveredDate, StoreId, ShipperId, ShippingLineId, ShippingCompanyId, ContainerSizeId, ContainerPrice, ContainerSizeCurrencyId, PolisaData, Perc, Cost_InvoiceNo, Cost_MessageId, Cost_OrderTypeId, Cost_Administrative, Bal0, val5}
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If ShipperId.Visibility = Windows.Visibility.Visible AndAlso ShipperId.Text.Trim = "" Then
            bm.ShowMSG("برجاء تحديد " & lblShipperId.Content)
            ShipperId.Focus()
            Return
        End If
        If ShippingLineId.Visibility = Windows.Visibility.Visible AndAlso ShippingLineId.Text.Trim = "" Then
            bm.ShowMSG("برجاء تحديد " & lblShippingLineId.Content)
            ShippingLineId.Focus()
            Return
        End If
        If ShippingCompanyId.Visibility = Windows.Visibility.Visible AndAlso ShippingCompanyId.Text.Trim = "" Then
            bm.ShowMSG("برجاء تحديد " & lblShippingCompanyId.Content)
            ShippingCompanyId.Focus()
            Return
        End If
        If ContainerSizeId.Visibility = Windows.Visibility.Visible AndAlso ContainerSizeId.Text.Trim = "" Then
    ...
    Private Sub txtID_KeyUp(sender As Object, e As KeyEventArgs) Handles txtID.KeyUp
        Try
            If bm.ShowHelp(TableName, txtID, txtID, e, "select cast(Id as varchar(100)) المسلسل,dbo.GetShipperName(ShipperId) الشاحن from " & TableName & " where " & MainId & "=" & OrderTypeId.Text, , , , "رقم الرسالة", "Shipper") Then
                DayDate.Focus()
            End If
        Catch ex As Exception
        End Try
    ...
    Private Sub txtID_KeyPress2(ByVal sender As Object, ByVal e As System.Windows.Input.KeyEventArgs) Handles ContainerPrice.KeyDown
    ...
    Private Sub FillControls()
        ...
        MainAccNo_LostFocus(Nothing, Nothing)
        StoreId_LostFocus(StoreId, Nothing)
        Dim dt As DataTable = bm.ExecuteAdapter("select * from " & TableNameDetails & " where OrderTypeId='" & OrderTypeId.Text & "' and Id='" & Val(txtID.Text) & "'")
        G.Rows.Clear()
        For i As Integer = 0 To dt.Rows.Count - 1
            G.Rows.Add()
    ...
    Private Sub IsDelivered_Checked(sender As Object, e As RoutedEventArgs) Handles IsDelivered.Checked
        lblToDate.Visibility = Windows.Visibility.Visible
        DeliveredDate.Visibility = Windows.Visibility.Visible
        btnCalc.Visibility = Windows.Visibility.Visible
        btnCalc2.Visibility = Windows.Visibility.Visible
        lblPerc.Visibility = Windows.Visibility.Visible
        Perc.Visibility = Windows.Visibility.Visible
    End Sub
    ...
    Private Sub IsDelivered_UnChecked(sender As Object, e As RoutedEventArgs) Handles IsDelivered.Unchecked
        ...
        DeliveredDate.Visibility = Windows.Visibility.Hidden
        btnCalc.Visibility = Windows.Visibility.Hidden
        btnCalc2.Visibility = Windows.Visibility.Hidden
    ...
    Private Sub GridCalcRow(sender As Object, e As Forms.DataGridViewCellEventArgs)
        ...
        If G.Columns(e.ColumnIndex).Name = GC.InvoiceNo Then
            ...
            dt = bm.ExecuteAdapter("select ToId,dbo.GetSupplierName(ToId)ToName,DocNo,dbo.ToStrDate(DeliveryDate)DeliveryDate,Perc from SalesMaster where StoreId=" & StoreId.Text & " and Flag=" & Sales.FlagState.الاستيراد & " and InvoiceNo=" & G.Rows(e.RowIndex).Cells(GC.InvoiceNo).Value)
            If dt.Rows.Count > 0 Then
...
a:
    ...
    Private Sub G1_CellDoubleClick(sender As Object, e As Forms.DataGridViewCellEventArgs)
        ...
        Else
            ...
            If i = 0 Then
                G2.Rows(i).Cells(GC2.SerialNo).Value = Val(bm.ExecuteScalar("select dbo.GetSalesNewSerial(" & StoreId.Text & ",1)"))
            Else
    ...
    Private Sub BtnPrint20_Click(sender As Object, e As RoutedEventArgs) Handles btnPrint20.Click
        DontClear = True
        btnSave_Click(sender, Nothing)
        DontClear = False
        Dim rpt As New ReportViewer
        rpt.Rpt = "ImportMessages20.rpt"
        rpt.paraname = New String() {"@OrderTypeId", "@Id", "Header"}
        rpt.paravalue = New String() {Val(OrderTypeId.Text), Val(txtID.Text), sender.Content.ToString}
        rpt.Show()
    End Sub
End Class
